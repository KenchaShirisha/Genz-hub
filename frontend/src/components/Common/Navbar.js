...
export default function Navbar() {
  ...
  return (
    <>
      <style>{`
        .nb-desktop { display: flex; gap: 4px; flex: 1; flex-wrap: nowrap; }
        .nb-hamburger { display: none; }
        .nb-logout { display: inline-flex; }
      ...
      <nav style={styles.nav}>
        ...
        <div style={styles.right}>
          ...
          <button onClick={handleLogout} className="nb-logout" style={{ ...styles.iconBtn, color: '#ef4444' }}>Logout</button>
          <button className="nb-hamburger" style={styles.iconBtn} onClick={() => setMenuOpen(!menuOpen)}>
            {menuOpen ? '✕' : '☰'}
          </button>
        </div>
      </nav>
...
const styles = {
  ...
  iconBtn: { background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, padding: '6px 10px', borderRadius: 8, color: 'var(--text)', fontWeight: 500, alignItems: 'center' },
  ...
  mobileMenu: { position: 'fixed', top: 60, left: 0, right: 0, background: 'var(--bg2)', borderBottom: '2px solid var(--border)', zIndex: 99, display: 'flex', flexDirection: 'column', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' },
  ...
  mobileLogout: { padding: '14px 20px', fontSize: 15, fontWeight: 600, color: '#ef4444', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer' }
};
